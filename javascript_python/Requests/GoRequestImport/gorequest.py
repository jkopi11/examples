...
try:
    ...
    def processActions():
        ...
        goRequestFile = r"/Users/josephkopinski/Development/RequestsGitDev/GoRequestImport/GoRequestRequests.csv"
            ...
            with open(goRequestFile) as f:
                ...
                for line in f:
                    ...
                    if count == 0:
                        ...
                        data = line.split(',')
                        d = {}
                        if len(data[headers.index('Entered date')]) > 0:
                            address = data[headers.index('Location')].split('-')[0].replace('"','')
                            location = geocodeAddress(address)
                            if location:
                                ...
                                #'RequestID','Description','RequestTypeText','Address','AddressNo','StreetName','X','Y','EmployeeID','EmployeeText','DeptID','DepartmentText','UpdatedBy','StatusText','StatusCode','RequestDate','SubmittedDate','StatusDate'
                                requestTypeText = requesttypes[data[headers.index('Topic')]]
                                p = address.find(' ')
                                addressNo = address[0:p]
                                streetName = address[p+1:]                               
                                #notes = data[headers.index('Internal Notes\r')].replace(':\r','')
                                #notes = data[headers.index('Reason closed\r')].rstrip().rstrip('\r\n')
                                if len(data[headers.index('Date closed\r')].rstrip()) > 0:
                                    statusDate = datetime.datetime.strptime(data[headers.index('Date closed\r')].rstrip(),'%m/%d/%Y %H:%M')
                                else:
                                    statusDate = requestDate
                                emp = data[headers.index('Assigned to')]
                                actionArray.append([requestID,data[headers.index('Description')],requestTypeText,address,addressNo,streetName,location[0],location[1],employeeID[emp],employeeName[emp],'3','Community Development',employees[emp],status[data[headers.index('Status')]],statusCode[data[headers.index('Status')]],requestDate.strftime('%m/%d/%Y %I:%M:%S %p'),requestDate.strftime('%m/%d/%Y %I:%M:%S %p'),statusDate.strftime('%m/%d/%Y %I:%M:%S %p')])
                            else:
                                print 'ERROR: ' + data[headers.index('ID')] + ' ----- ERROR ----- '
                    ...
                    count = count + 1
                actionFile = './GoRequestRequestsProcessed.csv'
